...
def model_averager(model_locations):
    ...
    model = transformers.AutoModel.from_pretrained(model_locations[0])
    ...
    for location in model_locations:
        ...
        model = transformers.AutoModel.from_pretrained(location)
...
def path_parser(models_dir):
  ...
  for root, dirs, files in os.walk(models_dir):
    for file in files:
      if (('model' in file) and (file.endswith('.bin') | file.endswith('.safetensors'))) or os.path.isdir(os.path.join(root, file)):
