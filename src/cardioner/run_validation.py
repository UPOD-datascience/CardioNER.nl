...
from utils import clean_spans, merge_annotations, process_pipe
...
def main(
    ...
    custom_predictor: bool = False,
    device="cpu",
    aggregation_method: Literal[
        "simple", "first", "max", "average", "wesam"
    ] = "simple",
    confidence_threshold: float = 0.35,
    use_pre_chunker: bool = False,
    post_hoc_cleaning=True,
    **kwargs,
):
    sample_list = merge_annotations(
        annotation_directory=input_dir, annotation_tsv=annotation_tsv
    )
    ...
    if aggregation_method in ["simple", "first", "max", "average"]:
        if aggregation_method == "simple":
            ...
            tokenizer = AutoTokenizer.from_pretrained(
                model, truncation=True, padding="max_length"
            )
            le_pipe = pipeline(
            ...
            try:
                ...
                print(
                    "word_ids() is NOT supported by this tokenizer. Cannot continue with this aggregation_strategy"
                )
            ...
            print(
                "Pipeline’s tokenizer.model.continuing_subword_prefix:",
                tokenizer._tokenizer.model.continuing_subword_prefix,
            )

            assert tokenizer.is_fast, (
                "The tokenizer must be Fast, otherwise it does not contain word_ids"
            )
            ...
            if (tokenizer._tokenizer.model.continuing_subword_prefix is None) | (
                ...
                print(
                    "The tokenizer does not have continuing_subword_prefix, setting it manually.."
                )

            sample_word = (
                "HuggingFace"  # for RoBERTa, often splits to ["Hugging", "Face"]
            )
        ...
        if batchwise == False:
            ...
            if aggregation_method == "simple":
                ...
                raise ValueError(
                    "We have no support yet for batchwise inference with non-simple aggregation."
                )

            results = process_pipe(
                text=ner_dataset,
                lang=lang,
                pipe=le_pipe,
                max_word_per_chunk=stride,
            ...
            for i, doc_results in enumerate(results):
                if len(doc_results) > 0:
                    ...
                    _res_df["id"] = sample_list[i][
                        "id"
                    ]  # Get ID from the original sample_list
                    if ignore_zero:
                        _res_df = _res_df[_res_df.entity_group != "LABEL_0"]
                    res_df_raw = pd.concat([res_df_raw, _res_df], axis=0)
    else:
    ...
    if split_by_class:
        ...
        res_df_raw[
            ["filename", "ann_id", "label", "start_span", "end_span", "text"]
        ].to_csv(f"{file_prefix}results.tsv", sep="\t", index=False)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test the trained model, input a directory with jsonls, outputs a tsv with results"
    )
    parser.add_argument(
        "--model",
        type=str,
        help="The model to test, can be a path or a model name",
        default="UMCU/CardioNER.nl_128",
    )
    parser.add_argument(
        "--revision", type=str, help="Model revision, optional", default=None
    )
    parser.add_argument(
        "--lang",
        type=str,
    ...
    parser.add_argument(
        "--aggregation_method",
        type=str,
        choices=["simple", "max", "first", "average", "wesam"],
        default="simple",
    )
    ...
    parser.add_argument(
        "--annotation_tsv",
        type=str,
        help="Annotation file, only for folder with txts",
        default=None,
    )
